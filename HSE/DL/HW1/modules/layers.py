...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            self.var = input.var(axis=0)  # biased estimate
            
            # Update running statistics
            self.running_mean = (1 - self.momentum) * self.running_mean + self.momentum * self.mean
            # Correct for unbiased variance
            B = input.shape[0]
            unbiased_var = self.var * B / (B - 1) if B > 1 else self.var
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            # gradient formula for batch normalization
            # grad_x = (1/B) * inv_std * (B * grad_x_hat - sum(grad_x_hat) - x_hat * sum(grad_x_hat * x_hat))
            sum_grad_norm = grad_norm.sum(axis=0)
            sum_grad_norm_xhat = (grad_norm * self.norm_input).sum(axis=0)
            
            grad_input = (1.0 / B) * self.inv_sqrt_var * (
                B * grad_norm - sum_grad_norm - self.norm_input * sum_grad_norm_xhat
            )
        else:
            # Evaluation mode: simple scaling
            grad_input = grad_norm / np.sqrt(self.running_var + self.eps)
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        # Store intermediate inputs for backward pass
        inputs = [input]
        x = input
        ...
        grad = grad_output
        for module, module_input in zip(reversed(self.modules), reversed(inputs)):
