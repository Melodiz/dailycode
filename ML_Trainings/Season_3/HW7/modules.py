...
Original file is located at
    https://colab.research.google.com/drive/1H7cwB9pIJAWNAAvgp7s5i-hz2KcnND0v
...
class Sequential(Module):
    ...
    def backward(self, input, gradOutput):
        ...
        # First, compute all intermediate inputs
        inputs = [input]
        current_input = input
        
        for i in range(len(self.modules) - 1):
            current_input = self.modules[i].forward(current_input)
...
class Linear(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        # Gradient with respect to input: gradInput = gradOutput * W
        self.gradInput = np.dot(gradOutput, self.W)
        return self.gradInput

    def accGradParameters(self, input, gradOutput):
        # Your code goes here. ################################################
        # Gradient with respect to weights: gradW = gradOutput^T * input
        # Gradient with respect to bias: gradb = sum(gradOutput, axis=0)
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(len(input)):
            # Jacobian matrix of softmax: diag(softmax) - outer_product(softmax, softmax)
            jacobian = np.diag(softmax_output[i]) - np.outer(softmax_output[i], softmax_output[i])

            # Multiply Jacobian by gradient output for this sample
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = self.output - np.log(np.sum(exp_values, axis=1, keepdims=True))

        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        softmax_output = np.exp(self.output)  # exp(log-softmax) = softmax

        # The gradient of log-softmax is: gradInput = gradOutput - softmax * sum(gradOutput)
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        # Initialize statistics if they don't exist yet
        if self.moving_mean is None:
            self.moving_mean = np.zeros(input.shape[1])
            self.moving_variance = np.zeros(input.shape[1])
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        if self.training:
            ...
            dx_normalized = gradOutput
            
            # Gradient with respect to variance
            dvar = np.sum(dx_normalized * self.x_centered * -0.5 * np.power(self.std, -3), axis=0)
            
            # Gradient with respect to mean
            dmean = np.sum(dx_normalized * -1.0 / self.std, axis=0)
            dmean += dvar * np.sum(-2.0 * self.x_centered, axis=0) / batch_size
            
            # Gradient with respect to input
            self.gradInput = dx_normalized / self.std
            self.gradInput += dvar * 2.0 * self.x_centered / batch_size
            self.gradInput += dmean / batch_size
            
        else:
            # During evaluation, just scale by the standard deviation
...
class Dropout(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        if self.training:
            # using the same mask, zero out the gradients on zeroed elements
...
class SoftPlus(Module):
    ...
    def updateOutput(self, input):
        # Your code goes here. ################################################
        threshold = 20  # Beyond this value, exp(x) might overflow

        self.output = np.zeros_like(input)

        # use approximation ln(1 + e^x) = x
        mask_pos = input > threshold # prevent overflow
        self.output[mask_pos] = input[mask_pos]

        mask_neg = input < -threshold # prevent underflow, use the approximation exp(x)
        self.output[mask_neg] = np.exp(input[mask_neg])

        mask_mid = ~(mask_pos | mask_neg) # for the rest, apply wiki formula
        self.output[mask_mid] = np.log(1 + np.exp(input[mask_mid]))

        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        # The derivative of SoftPlus is the sigmoid function: f'(x) = 1/(1 + e^(-x)) (wiki)
        sigmoid = 1.0 / (1.0 + np.exp(-input))
        self.gradInput = sigmoid * gradOutput
...
class ClassNLLCriterionUnstable(Criterion):
    EPS = 1e-15
    def __init__(self):
        a = super(ClassNLLCriterionUnstable, self) # Who the fuck make this line???
    ...
    def updateOutput(self, input, target):
        ...
        # Your code goes here. ################################################
        batch_size = input.shape[0]
        selected_probs = input_clamp * target
        log_probs = -np.log(np.sum(selected_probs, axis=1)) # sum over all classes
        self.output = np.sum(log_probs) / batch_size # take avarage
    ...
    def updateGradInput(self, input, target):
        ...
        # Your code goes here. ################################################
        batch_size = input.shape[0]
        self.gradInput = np.zeros_like(input_clamp)

        # The gradient is -1/p for the correct class, 0 for others
        self.gradInput = np.where(target > 0, -1.0 / input_clamp, 0)
...
class ClassNLLCriterion(Criterion):
    ...
    def updateOutput(self, input, target):
        # Your code goes here. ################################################
        batch_size = input.shape[0]
        selected_log_probs = input * target

        log_probs = np.sum(selected_log_probs, axis=1)
        self.output = -np.sum(log_probs) / batch_size

        return self.output

    def updateGradInput(self, input, target):
        # Your code goes here. ################################################
        # The grad of -log(p) with respect to log(p) is -1 for the correct class
        batch_size = input.shape[0]
        self.gradInput = np.zeros_like(input)
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        # Compute convolution for each sample in the batch and each output channel
        for b in range(batch_size):
            for out_ch in range(self.out_channels):
                # Correlate the input with the filter and add bias
                self.output[b, out_ch] = scipy.signal.correlate(
                    padded_input[b],
                    self.W[out_ch],
                    mode='valid'
                ) + self.b[out_ch]
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # Compute grads with respect to input
        for b in range(batch_size):
            for in_ch in range(in_channels):
                for out_ch in range(self.out_channels):
                    # Correlate the gradOutput with the flipped filter
                    self.gradInput[b, in_ch] += scipy.signal.correlate(
                        padded_gradOutput[b, out_ch:out_ch+1],
                        flipped_W[out_ch, in_ch:in_ch+1],
                        mode='valid'
                    )[0]
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        for out_ch in range(self.out_channels):
            for in_ch in range(self.in_channels):
                for b in range(batch_size):
                    # Correlate the input with gradOutput to get gradW
                    self.gradW[out_ch, in_ch] += scipy.signal.correlate(
                        padded_input[b, in_ch:in_ch+1],
                        gradOutput[b, out_ch:out_ch+1],
                        mode='valid'
                    )[0]
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        # YOUR CODE #############################
        batch_size, n_channels = input.shape[0], input.shape[1]
        output_h, output_w = input_h // self.kernel_size, input_w // self.kernel_size

        # Reshape input to separate pooling regions
        x_reshaped = input.reshape(batch_size, n_channels,
                                  output_h, self.kernel_size,
                                  output_w, self.kernel_size)

        x_transposed = x_reshaped.transpose(0, 1, 2, 4, 3, 5)

        self.output = np.max(x_transposed, axis=(4, 5))
        self.x_transposed = x_transposed
        self.max_indices = np.zeros_like(x_transposed, dtype=bool)

        for b in range(batch_size):
            for c in range(n_channels):
                for h in range(output_h):
                    for w in range(output_w):
                        pool_region = x_transposed[b, c, h, w]
                        max_val = np.max(pool_region)
                        mask = (pool_region == max_val)
                        self.max_indices[b, c, h, w] = mask
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        grad_reshaped = self.gradInput.reshape(batch_size, n_channels,
                                              output_h, self.kernel_size,
                                              output_w, self.kernel_size)
        grad_transposed = grad_reshaped.transpose(0, 1, 2, 4, 3, 5)

        for b in range(batch_size):
            for c in range(n_channels):
                for h in range(output_h):
                    for w in range(output_w):
                        grad_val = gradOutput[b, c, h, w]

                        mask = self.max_indices[b, c, h, w]
                        num_maxes = np.sum(mask)
                        grad_transposed[b, c, h, w][mask] += grad_val / num_maxes
