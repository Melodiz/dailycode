...
def edit_mass(els, end_watching: Optional[datetime] = None):
...
while True:
    try:
        if els[i] >= start_watching and els[i+1] <= end_watching:
            x += int((els[i+1]-els[i]).total_seconds())
        if els[i] < start_watching and els[i+1] > end_watching:
            x += int((end_watching-start_watching).total_seconds())
        if els[i] < start_watching and els[i+1] <= end_watching and els[i+1] > start_watching:
            x += int((els[i+1]-start_watching).total_seconds())
        if els[i] >= start_watching and els[i+1] > end_watching and els[i] < end_watching:
            x += int((end_watching-els[i]).total_seconds())
