...
Complex operator*(double num, const Complex& obj) {
  Complex res;
  res.re = num * obj.re;
  res.im = num * obj.im;
  return res;
...
Complex operator/(double num, const Complex& obj) {
  Complex res;
  double denominator = pow(obj.re, 2) + pow(obj.im, 2);
  res.re = (num * obj.re) / denominator;
  res.im = (-num * obj.im) / denominator;
  return res;
}
...
bool operator!=(const Complex& n1, const Complex& n2) { return !(n1 == n2); }
...
void test(Complex num1, Complex num2) {
  ...
  assert((num1 + num2).re - real(n1 + n2) < dif &&
         (num1 + num2).im - imag(n1 + n2) < dif);
  ...
  assert((num1 / num2).re - real(n1 / n2) < dif &&
         (num1 / num2).im - imag(n1 / n2) < dif);
  assert((num2 / num1).re - real(n2 / n1) < dif &&
         (num2 / num1).im - imag(n2 / n1) < dif);
  std::cout << "Checkpoint #4\n";

  assert((num1 - 5.43).re - real(n1 - 5.43) < dif &&
         (num1 - 5.43).im - imag(n1 - 5.43) < dif);
  ...
  assert(((4.1 * num1).re - real(4.1 * n1) < dif) &&
         ((4.1 * num1).im - imag(4.1 * n1) < dif));
  assert(((4.1 / num1).re - real(4.1 / n1) < dif) &&
         ((4.1 / num1).im - imag(4.1 / n1) < dif));
  ...
  // std::cout << (4.1 * num1).re  << ' ' << real(4.1 * n1) << std::endl;
